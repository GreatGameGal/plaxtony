...
function translateNodeKind(node: Types.Node): lsp.SymbolKind {
    switch (node.kind) {
        case Types.SyntaxKind.VariableDeclaration:
            ...
            const isConstant = variable.modifiers.some(
                (value: Types.Modifier): boolean => {
...
function translateDeclaratons(
    ...
    for (let node of origDeclarations) {
        const sourceFile = <Types.SourceFile>(
            findAncestor(node, (element: Types.Node): boolean => {
...
export class Server {
    ...
    public createConnection() {
        ...
        this.connection.onRequest(
            "document/checkRecursively",
            this.onDiagnoseDocumentRecursively.bind(this),
        );
    ...
    private async reindex() {
        ...
        if (projFolders.length) {
                ...
                if (path.isAbsolute(this.config.archivePath)) {
                    if (await fs.pathExists(archivePath)) {
                        ...
                        this.showErrorMessage(
                            `Specified archivePath '${this.config.archivePath}' resolved to '${archivePath}', but it doesn't exist.`,
                        );
                    }
                } else {
                    const archiveOsNormalPath = osNormalizePath(
                        this.config.archivePath,
                    );
                    const candidates = (
                        await Promise.all(
                            projFolders.map(async (x) => {
                                const testedPath = path.join(
                    ...
                    if (candidates.length) {
                        ...
                        logger.info(
                            `Configured archivePath '${archiveOsNormalPath}' resolved to ${archivePath}`,
                        );
                        ...
                        this.showErrorMessage(
                            `Specified archivePath '${archiveOsNormalPath}' couldn't be found.`,
                        );
                    }
                }
            }
            ...
            if (!archivePath) {
                const cfgFilesExclude: { [key: string]: boolean } =
                    await this.connection.workspace.getConfiguration(
                        "files.exclude",
                    );
                const excludePatterns = Object.entries(cfgFilesExclude)
                    .filter((x) => x[1] === true)
                    .map((x) => x[0]);
                    ...
                    await Promise.all(
                        projFolders.map(async (wsFolder) => {
                            return {
                                wsFolder,
                                foundArchivePaths:
                                    await findSC2ArchiveDirectories(
            ...
            if (!archivePath) {
                const s2maps = s2archives.filter(
                    (v) => path.extname(v).toLowerCase() === ".sc2map",
                );
                ...
                if (s2maps.length === 1) {
                    ...
                    this.connection.window.showInformationMessage(
                        'Couldn\'t find applicable sc2map/sc2mod in the workspace. Set it manually under "sc2galaxy.archivePath" in projects configuration.',
                    );
                }
            }
        }
        ...
        if (this.config.dataPath) {
            if (path.isAbsolute(this.config.dataPath)) {
                ...
                for (const wsFolder of projFolders) {
                    const resolvedDataPath = path.join(
                        URI.parse(wsFolder.uri).fsPath,
        ...
        if (!modSources.length) {
            modSources.push(
                this.initParams.initializationOptions.defaultDataPath,
            );
        }
        ...
        if (archivePath) {
            ...
            const name = matchingWsFolder
                ? path.relative(
                      URI.parse(matchingWsFolder.uri).fsPath,
                      archivePath,
                  )
        ...
        const fallbackDep = new SC2Archive(
            this.config.fallbackDependency,
            await resolveArchiveDirectory(
                this.config.fallbackDependency,
                modSources,
            ),
        );
        const depLinks = await resolveArchiveDependencyList(
            wsArchive ? wsArchive : fallbackDep,
            modSources,
            {
                ...
                fallbackResolve: async (depName: string) => {
                    while (depName.length) {
                        for (const wsFolder of projFolders) {
                            ...
                            if (
                                depName.toLowerCase() ===
                                wsFolder.name.toLowerCase()
                            ) {
                                return fsPath;
                            } else {
                                const result = await resolveArchiveDirectory(
                                    depName,
                                    [fsPath],
                                );
        ...
        for (const [name, src] of mapFromObject(this.config.s2mod.extra)) {
            if (!fs.existsSync(src)) {
                this.showErrorMessage(
                    `Extra archive [${name}] '${src}' doesn't exist. Skipping.`,
                );
                continue;
            }
            depLinks.list.push({
        ...
        if (depLinks.unresolvedNames.length > 0) {
            this.showErrorMessage(
                `Some SC2 archives couldn't be found [${depLinks.unresolvedNames
                    .map((s) => `'${s}'`)
                    .join(
                        ", ",
                    )}]. By a result certain intellisense capabilities might not function properly.`,
            );
        }

        let depList = depLinks.list.map(
            (item) => new SC2Archive(item.name, item.src),
        );
        if (
            !wsArchive &&
            !depList.find((item) => item.name === fallbackDep.name)
        ) {
        ...
        await this.store.rebuildS2Metadata(
            this.config.metadata,
            this.config.dataCatalog,
        );
    ...
    @logIt({
        level: "verbose",
        profiling: false,
        argsDump: (ev) => ev.settings.sc2galaxy,
    })
    private onDidChangeConfiguration(ev: lsp.DidChangeConfigurationParams) {
        ...
        if (
            ...
            JSON.stringify(this.config.trace) !==
                JSON.stringify(newConfig.trace) ||
            JSON.stringify(this.config.metadata) !==
                JSON.stringify(newConfig.metadata) ||
            JSON.stringify(this.config.dataCatalog) !==
                JSON.stringify(newConfig.dataCatalog) ||
            JSON.stringify(this.config.s2mod) !==
                JSON.stringify(newConfig.s2mod)
        ) {
        ...
        switch (this.config.completion.functionExpand) {
            case "None":
                this.completionsProvider.config.functionExpand =
                    CompletionFunctionExpand.None;
                break;
            case "Parenthesis":
                this.completionsProvider.config.functionExpand =
    ...
    private async onDidChangeContent(
        ev: lsp.TextDocumentChangeEvent<TextDocument>,
    ) {
        ...
        if (!this.indexing && this.ready) {
            req.timer = setTimeout(
                this.onUpdateContent.bind(this, ev.document.uri, req),
    ...
    @logIt({
        level: "verbose",
        argsDump: (docUri: string, req: DocumentUpdateRequest) => {
            return { uri: docUri, ver: req.version };
        },
    })
    private async onUpdateContent(
        documentUri: string,
        req: DocumentUpdateRequest,
    ) {
        req.promise = new Promise((resolve) => {
            ...
            const diagDelay = this.config.documentDiagnosticsDelay
                ? this.config.documentDiagnosticsDelay
                : 1;
            if (
                this.config.documentDiagnosticsDelay !== false ||
                !req.isDirty
            ) {
                setTimeout(
                    this.onDiagnostics.bind(this, documentUri, req),
                    req.isDirty ? diagDelay : 1,
                );
            }
            resolve(true);
        });
    ...
    private onDiagnostics(documentUri: string, req: DocumentUpdateRequest) {
        ...
        this.connection.sendDiagnostics({
            uri: documentUri,
            diagnostics:
                this.diagnosticsProvider.provideDiagnostics(documentUri),
        });
    }
    ...
    @logIt({
        level: "verbose",
        argsDump: (ev: lsp.TextDocumentChangeEvent<TextDocument>) => {
            return { uri: ev.document.uri, ver: ev.document.version };
        },
    })
    ...
    private async onCompletion(params: lsp.TextDocumentPositionParams) {
        ...
        try {
            if (
                this.initParams.capabilities.textDocument.completion
                    .contextSupport
            ) {
        ...
        return this.completionsProvider.getCompletionsAt(
            ...
            getPositionOfLineAndCharacter(
                this.store.documents.get(params.textDocument.uri),
    ...
    private onDocumentSymbol(
        ...
        return translateDeclaratons(
            this.navigationProvider.getDocumentSymbols(params.textDocument.uri),
        );
    }
    ...
    private onWorkspaceSymbol(
        ...
        if (!this.ready) return null;
        return translateDeclaratons(
            this.navigationProvider.getWorkspaceSymbols(params.query),
        );
    }
    ...
    private async onDiagnoseDocumentRecursively(
        ...
        const dtotal = this.diagnosticsProvider.checkFileRecursively(
            params.uri,
        );
