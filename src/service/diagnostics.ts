...
export function formatDiagnosticTotal(summary: DiagnosticWorkspaceSummary) {
    ...
    for (const [uri, dlist] of summary.diagnostics) {
        ...
        for (const dg of dlist) {
            so.push(
                `\n[${DiagnosticCategory[dg.category].toUpperCase()}] ${
                    dg.messageText
                }`,
            );
            so.push(
                `\n    in ${URI.parse(uri).fsPath}:${dg.line + 1}:${
                    dg.col
                }\n\n`,
            );
        }
    ...
    for (const item of Object.keys(DiagnosticCategory).filter(
        (v) => typeof (DiagnosticCategory as any)[v] === "number",
    )) {
        so.push("=");
        so.push(
            summary.issuesTotal[
                DiagnosticCategory[item as keyof typeof DiagnosticCategory]
            ]
                .toString()
                .padStart(6),
        );
