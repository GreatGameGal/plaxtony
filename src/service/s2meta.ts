...
export class S2WorkspaceMetadata {
    ...
    public getElementSymbolName(el: trig.Element) {
        ...
        if (el.name) {
            ...
            const localizedName =
                this.workspace.locComponent.triggers.elementName("Name", el);
            if (localizedName) {
        ...
        if (
            el instanceof trig.FunctionDef &&
            (el.flags & trig.ElementFlag.Native ||
                el.flags & trig.ElementFlag.NoScriptPrefix)
        ) {
            parts.push(elemName);
        } else if (
            ...
            if (parts.length || el.constructor === trig.PresetValue) {
                ...
                parts.push(
                    elemName.charAt(0).toLowerCase() + elemName.substr(1),
                );
            }
        }
    ...
    private mapContainer(container: trig.ElementContainer) {
        for (const el of container.getElements().values()) {
            if (el instanceof trig.FunctionDef) {
                ...
                if (
                    !(el.flags & trig.ElementFlag.PresetGenConstVar) &&
                    !(el.flags & trig.ElementFlag.PresetCustom)
                )
                    continue;
                ...
                for (const presetRef of (<trig.Preset>el).values) {
                    ...
                    if (
                        !pname ||
                        pname === "null" ||
                        !pname.match(elementValidCharsRE)
                    ) {
                        continue;
                    }
    ...
    public async build() {
        ...
        this.workspace.metadataArchives = this.workspace.allArchives.filter(
            (item) => {
        ...
        logger.info(
            "metadata archives",
            ...this.workspace.metadataArchives.map((item) => item.name),
        );
        ...
        for (const lib of this.workspace.trigComponent
            .getStore()
            .getLibraries()
            .values()) {
    ...
    public getNameOfPresetValue(
        preset: trig.Preset,
        presetValue: trig.PresetValue,
    ) {
        ...
        if (preset.flags & trig.ElementFlag.PresetCustom) {
            ...
            return (
                this.getElementSymbolName(preset) +
                "_" +
                this.getElementSymbolName(presetValue)
            );
        }
    }
    ...
    public getParameterTypeDoc(el: trig.ParameterType) {
        ...
        if (el.type === "gamelink") {
            ...
            typeName = this.workspace.locComponent.triggers.elementName(
                "Name",
                el.typeElement.resolve(),
            );
            type = `Preset<${this.getElementSymbolName(
                el.typeElement.resolve(),
            )}>`;
        } else {
            type = el.type;
    ...
    public getParamDoc(el: trig.ParamDef) {
        const name = this.workspace.locComponent.triggers.elementName(
            "Name",
            el,
        );
    ...
    public getElementDoc(el: trig.Element, extended: boolean) {
        ...
        if (el instanceof trig.FunctionDef) {
            if (extended) {
                const grammar =
                    this.workspace.locComponent.triggers.elementName(
                        "Grammar",
                        el,
                    );
                if (grammar) {
            ...
            const hint = this.workspace.locComponent.triggers.elementName(
                "Hint",
                el,
            );
            if (hint) {
            ...
            const presetName = this.workspace.locComponent.triggers.elementName(
                "Name",
                this.findPresetDef(el),
            );
            ...
            if ((<trig.ParamDef>el).type.type === "gamelink") {
                type =
                    "`gamelink<" +
                    ((<trig.ParamDef>el).type.gameType || "any") +
                    ">`";
                ...
                type =
                    ...
                    this.workspace.locComponent.triggers.elementName(
                        "Name",
                        (<trig.ParamDef>el).type.typeElement.resolve(),
                    ) +
                    "";
            } else {
    ...
    public getElementTypeOfNode(node: gt.Node) {
        ...
        const el = <trig.FunctionDef>(
            this.findElementByName((<gt.Identifier>callExpr.expression).name)
        );
        if (!el) return null;

        let index: number = null;
        if (
            node.kind === gt.SyntaxKind.CommaToken ||
            node.kind === gt.SyntaxKind.OpenParenToken
        ) {
    ...
    public getGameLinkItem(gameType: string, id?: string) {
        ...
        let results = Array.from(
            this.workspace.catalogComponent.getStore().findEntry(family),
        )
            .map((x) => Array.from(x))
            .flat();
        if (id) {
    ...
    public getGameLinkLocalizedName(
        gameType: string,
        gameLink: string,
        includePrefix: boolean = false,
    ) {
        const name =
            this.workspace.locComponent.strings
                .get("Game")
                .text(`${gameType}/Name/${gameLink}`) ??
            this.workspace.locComponent.strings
                .get("Object")
                .text(`${gameType}/Name/${gameLink}`);
        if (!includePrefix) {
        ...
        const prefix = this.workspace.locComponent.strings
            .get("Object")
            .text(`${gameType}/EditorPrefix/${gameLink}`);
        const suffix = this.workspace.locComponent.strings
            .get("Object")
            .text(`${gameType}/EditorSuffix/${gameLink}`);
        return (
            (prefix ? prefix + " " : "") +
            (name ?? gameLink) +
            (suffix ? " " + suffix : "")
...
export function getDocumentationOfSymbol(
    store: Store,
    symbol: gt.Symbol,
    ...
    for (const decl of symbol.declarations) {
        ...
        while (currLine > 0 && sourceFile.commentsLineMap.has(currLine)) {
            ...
            linesTxt.push(
                sourceFile.text.substring(ctoken.pos + 2, ctoken.end),
            );
            --currLine;
        }
        if (linesTxt.length) {
