...
(async function () {
    ...
    const rootArchive = new SC2Archive(
        "mods/core.sc2mod",
        path.join(process.argv[2], "mods/core.sc2mod"),
    );
    ...
    for (const filepath of await rootArchive.findFiles("**/*.galaxy")) {
        ...
        store.updateDocument(
            createTextDocumentFromFs(
                path.join(rootArchive.directory, filepath),
            ),
        );
    }
    ...
    for (const curEl of ntveLib.getElements().values()) {
        ...
        if (
            curEl.returnType &&
            curEl.returnType.type !== curEl.returnType.galaxyType
        ) {
            out.push(
                `///\n/// # Returns \`${
                    store.s2metadata.getParameterTypeDoc(curEl.returnType).type
                }\`\n`,
            );
        ...
        for (const [key, param] of curEl.getParameters().entries()) {
            ...
            if (isKeywordTypeKind(stringToToken(param.name.toLowerCase()))) {
                paramName = `in${paramName
                    .charAt(0)
                    .toUpperCase()}${paramName.slice(1)}`;
            }

            out.push("\n");
            out.push(
                `    ${param.galaxyType} ${paramName}${
                    key < curEl.parameters.length - 1 ? "," : ""
                }`.padEnd(35),
            );
            out.push(
                ` /// * ${paramDoc.name}`.padEnd(25) + ` :: ${paramDoc.type}`,
            );
