...
const reValidArchiveExtension = new RegExp(
...
const builtinDepsHierarchy = (function () {
    function depsFor(modName: builtinDepName) {
        ...
        if (
            (matches =
                /^campaigns\/(liberty|swarm|void)story\.sc2campaign$/i.exec(
                    modName,
                ))
        ) {
            ...
            (matches =
                /^mods\/(liberty|swarm|void|balance)multi\.sc2mod$/i.exec(
                    modName,
                ))
        ) {
            ...
            (matches = /^campaigns\/(liberty|swarm|void)\.sc2campaign$/i.exec(
                modName,
            ))
        ) {
            if (matches[1] === "void") {
            ...
            (matches = /^mods\/(liberty|swarm|void)\.sc2mod$/i.exec(modName))
        ) {
            if (matches[1] === "void") {
    ...
    for (const modName of Object.keys(BuiltinDeps).filter(
        (v) => typeof (BuiltinDeps as any)[v] === "number",
    )) {
        ...
        depHierarchy[modName as builtinDepName] = depHierarchy[
            modName as builtinDepName
        ].concat(Array.from(new Set(depsFor(modName as builtinDepName))));
    }
    return depHierarchy;
})();
...
export async function findSC2ArchiveDirectories(
    ...
    return results.sort((a, b) => {
        return (
            validArchiveExtensions.indexOf(
                b.match(reValidArchiveExtension)[1].toLowerCase(),
            ) -
            validArchiveExtensions.indexOf(
                a.match(reValidArchiveExtension)[1].toLowerCase(),
            )
        );
    });
}
...
export class CatalogComponent extends Component {
    ...
    @logIt()
    public async loadData() {
        for await (const [archive, filename] of this.workspace.findFiles(
            "**/GameData/**/*.xml",
        )) {
            logger.debug(`:: ${archive.name}/${filename}`);
            const doc = await SC2Workspace.documentFromFile(
                archive,
                filename,
                "xml",
            );
...
export class LocalizationComponent extends Component {
    ...
    private async loadStrings(name: string) {
        ...
        for (const archive of this.workspace.metadataArchives) {
            const filenames = await archive.findFiles(
                "**/" +
                    this.lang +
                    ".SC2Data/LocalizedData/" +
                    name +
                    "Strings.txt",
            );
            if (filenames.length) {
    ...
    public async loadData() {
        for (const archive of this.workspace.metadataArchives) {
            const filenames = await archive.findFiles(
                "**/" + this.lang + ".SC2Data/LocalizedData/TriggerStrings.txt",
            );
            if (filenames.length) {
                logger.debug(`:: ${archive.name}/${filenames[0]}`);
                const locFile = new loc.LocalizationFile();
...
export async function resolveArchiveDependencyList(
    rootArchive: SC2Archive,
    sources: string[],
    opts: ResolveDependencyOpts = {},
) {
...
export async function openArchiveWorkspace(
    archive: SC2Archive,
    sources: string[],
    ...
    if (result.unresolvedNames.length > 0) {
        throw new Error(
            `couldn\'t resolve ${util.inspect(
                result.unresolvedNames,
            )}\nSources: ${util.inspect(sources)}\nOverrides: ${util.inspect(
                overrides,
            )}`,
        );
    }
...
export class SC2Archive {
    ...
    public relativePath(uri: lsp.DocumentUri) {
        ...
        if (
            fsPath.substring(0, this.lcFsPath.length).toLowerCase() !==
            this.lcFsPath
        )
            return;
    ...
    public async getDependencyList() {
        ...
        if (builtinDepsHierarchy[this.name as builtinDepName]) {
            list = list.concat(
                builtinDepsHierarchy[this.name as builtinDepName],
            );
        }
        ...
        if (await this.hasFile("DocumentInfo")) {
            try {
                ...
                for (const depValue of data.DocInfo.Dependencies[0].Value) {
                    list.push(
                        depValue
                            .substr(depValue.indexOf("file:") + 5)
                            .replace(/\\/g, "/")
                            .toLowerCase(),
                    );
                }
                ...
                logger.warn(
                    `Couldn't read dependencies from "DocumentInfo" of "${this.name}`,
                    (<Error>err).message,
                );
...
const reArchiveFileNs =
    /^(?:(?<nsName>[a-z]+)\.(?<nsType>(?:sc2data|sc2assets))(?:\/|\\))?(?<rp>.+)$/i;
const reIsUri = /^[^:/?#]+:\/\//i;
...
export class SC2Workspace {
    ...
    public resolvePath(fsPath: lsp.URI): S2QualifiedFile | undefined {
        ...
        for (const cArchive of this.allArchives) {
            ...
            const m = fsPath
                .substring(cArchive.directory.length + 1)
                .match(reArchiveFileNs);
            if (!m) return;
            ...
            return {
                ...
                archiveRelpath: m.groups["nsName"]
                    ? `${m.groups["nsName"]}.${m.groups["nsType"]}/${relativePath}`
                    : relativePath,
                namespace: ns,
                archive: cArchive,
                priority: priority,
