...
export abstract class AbstractType implements gt.Type {
    ...
    public isValidBinaryOperation(
        operation: gt.BinaryOperator,
        rightType: AbstractType,
    ) {
        return false;
    }
    ...
    public isAssignableTo(target: AbstractType) {
        ...
        if (target instanceof IntrinsicType) {
            if (
                ...
                (this.flags & gt.TypeFlags.Integer ||
                    this.flags & gt.TypeFlags.Byte)
            )
                return true;
            if (
            ...
            if (
                ...
                return true;
            if (
                this.flags & gt.TypeFlags.Boolean &&
                target.flags & gt.TypeFlags.Boolean
            )
                return true;
        } else if (target instanceof ComplexType) {
            if (
                this.flags & gt.TypeFlags.String &&
                target.kind === gt.SyntaxKind.HandleKeyword
            )
                return true;
        }

        if (
            this.flags & gt.TypeFlags.Null &&
            target.flags & gt.TypeFlags.Nullable
        )
            return true;
    ...
    public isValidBinaryOperation(
        ...
        if (
            ...
            (rightType instanceof LiteralType &&
                rightType.value.kind === gt.SyntaxKind.StringLiteral)
        ) {
            switch (operation) {
        ...
        if (
            ...
            (rightType instanceof LiteralType &&
                rightType.value.kind === gt.SyntaxKind.NumericLiteral)
        ) {
            switch (operation) {
                case gt.SyntaxKind.PlusToken:
                ...
                case gt.SyntaxKind.CaretToken: {
                    ...
                    if (
                        (this.flags & gt.TypeFlags.IntLike) ===
                        (rightType.flags & gt.TypeFlags.IntLike)
                    ) {
                        return true;
                    }
                    break;
...
export class ComplexType extends AbstractType implements gt.ComplexType {
    ...
    public isAssignableTo(target: AbstractType): boolean {
        ...
        if (target instanceof ComplexType) {
            if (target.kind === gt.SyntaxKind.HandleKeyword)
                return this.extendsHandle;
            if (this.kind === gt.SyntaxKind.HandleKeyword)
                return target.extendsHandle;
        } else {
            if (
                this.kind === gt.SyntaxKind.HandleKeyword &&
                target.flags & gt.TypeFlags.String
            )
                return true;
        }
...
export class LiteralType extends AbstractType {
    ...
    public isAssignableTo(target: AbstractType) {
        ...
        if (
            this.value.kind === gt.SyntaxKind.StringLiteral &&
            target.flags & gt.TypeFlags.String
        ) {
            return true;
        }
        if (
            this.value.kind === gt.SyntaxKind.NumericLiteral &&
            (target.flags & gt.TypeFlags.Byte ||
                target.flags & gt.TypeFlags.Integer ||
                target.flags & gt.TypeFlags.Fixed)
        ) {
            if (
                this.value.text.indexOf(".") !== -1 &&
                !(target.flags & gt.TypeFlags.Fixed)
            ) {
                return false;
            }
    ...
    public isComparableTo(target: AbstractType) {
        ...
        if (
            target instanceof LiteralType &&
            this.value.kind === target.value.kind
        )
            return true;
...
export class StructType extends AbstractType implements gt.StructType {
    ...
    public isAssignableTo(target: AbstractType) {
        if (
            target instanceof ReferenceType &&
            target.kind === gt.SyntaxKind.StructrefKeyword &&
            this.symbol === (<StructType>target.declaredType).symbol
        ) {
            return true;
        }
        return false;
...
export class FunctionType extends AbstractType implements gt.FunctionType {
    ...
    public isAssignableTo(target: AbstractType): boolean {
        if (
            target instanceof ReferenceType &&
            target.kind === gt.SyntaxKind.FuncrefKeyword
        ) {
            if (!(target.declaredType.flags & gt.TypeFlags.Function))
                return false;
...
export type ReferenceKind =
    ...
    | gt.SyntaxKind.StructrefKeyword
...
export class ReferenceType extends AbstractType {
    ...
    public isComparableTo(target: AbstractType): boolean {
        ...
        if (target instanceof FunctionType) {
            if (this.kind === gt.SyntaxKind.FuncrefKeyword) {
                return (<FunctionType>this.declaredType).signature.match(
                    target.signature,
                );
            }
        }
    ...
    public getName() {
        return (
            tokenToString(this.kind) + "<" + this.declaredType.getName() + ">"
        );
    }
}
...
export class ArrayType extends AbstractType implements gt.ArrayType {
    ...
    public isAssignableTo(target: AbstractType): boolean {
        if (
            target instanceof ReferenceType &&
            target.kind === gt.SyntaxKind.ArrayrefKeyword
        ) {
            ...
            if (
                this.elementType ===
                (<ArrayType>target.declaredType).elementType
            )
                return true;
        }
        return false;
...
const stringType = new IntrinsicType(
    gt.TypeFlags.String | gt.TypeFlags.Nullable,
    "string",
);
...
function generateComplexTypes() {
    ...
    for (
        let i = gt.SyntaxKindMarker.FirstComplexType;
        i <= gt.SyntaxKindMarker.LastComplexType;
        i++
    ) {
...
export class TypeChecker {
    ...
    private report(
        location: gt.Node,
        msg: string,
        category: gt.DiagnosticCategory = gt.DiagnosticCategory.Error,
    ...
    private checkTypeBoolExpression(
        source: AbstractType,
        negation: boolean,
        node: gt.Node,
    ) {
        ...
        if (!source.isBoolExpression(negation)) {
            this.report(
                ...
                "Type '" +
                    source.getName() +
                    "' can not be used as boolean expression",
            );
        }
    }
    ...
    private getTypeFromArrayTypeNode(node: gt.ArrayTypeNode): ArrayType {
        ...
        if (!links.resolvedType) {
            links.resolvedType = new ArrayType(
                this.getTypeFromTypeNode(node.elementType),
            );
        }
    ...
    private getTypeOfTypedef(symbol: gt.Symbol): AbstractType {
        const refType = this.getTypeFromTypeNode(
            (<gt.TypedefDeclaration>symbol.declarations[0]).type,
        );
    ...
    private getTypeOfSymbol(symbol: gt.Symbol): AbstractType {
        if (
            symbol.flags & gt.SymbolFlags.Variable ||
    ...
    private getTypeOfVariableOrParameterOrProperty(
        symbol: gt.Symbol,
    ): AbstractType {
        return this.getTypeFromTypeNode(
            (<gt.VariableDeclaration>symbol.declarations[0]).type,
        );
    }

    public getTypeOfNode(
        node: gt.Node,
        followRef: boolean = false,
    ): AbstractType {
        // TODO:
    ...
    public checkSourceFile(sourceFile: gt.SourceFile, bindSymbols = false) {
        ...
        this.currentDocuments = this.store.documents;
        if (bindSymbols) {
            unbindSourceFile(sourceFile, {
                resolveGlobalSymbol: this.resolveGlobalSymbol.bind(this),
            });
            this.currentSymbolContainer = declareSymbol(
                sourceFile,
                { resolveGlobalSymbol: this.resolveGlobalSymbol.bind(this) },
                null,
            );
    ...
    protected checkSourceFileRecursivelyWorker(sourceFile: gt.SourceFile) {
        ...
        for (const statement of sourceFile.statements) {
            if (statement.kind === gt.SyntaxKind.IncludeStatement) {
                const qsFile = this.checkIncludeStatement(
                    <gt.IncludeStatement>statement,
                );
    ...
    public checkSourceFileRecursively(sourceFile: gt.SourceFile) {
        ...
        if (this.store.s2workspace) {
            const coreMod = this.store.s2workspace.allArchives.find(
                (archive) => archive.name === "mods/core.sc2mod",
            );
            if (coreMod) {
                ...
                for (const fpath of nativeScripts) {
                    const qFile = this.store.documents.get(
        ...
        return {
            success:
                Array.from(this.diagnostics.values()).findIndex(
        ...
        for (const [symbol, symRef] of this.currentSymbolReferences) {
            if (symbol.flags & gt.SymbolFlags.Function) {
                ...
                for (const identifier of symRef) {
                    this.report(
                        identifier,
                        `Referenced function '${identifier.name}' hasn't been defined.`,
                    );
                }
            }
        }
    ...
    private checkIsSymbolDeclarationDefined(symbol: gt.Symbol) {
        ...
        if (symbol.flags & gt.SymbolFlags.Variable) {
            ...
            if (
                (symbol.flags & gt.SymbolFlags.GlobalVariable &&
                    !(symbol.flags & gt.SymbolFlags.Static)) ||
                (symbol.flags & gt.SymbolFlags.LocalVariable &&
                    !(symbol.parent?.flags & gt.SymbolFlags.Function)) ||
                (symbol.flags & gt.SymbolFlags.FunctionParameter &&
                    symbol.parent?.flags & gt.SymbolFlags.Function)
            ) {
                return;
            }
    ...
    private checkForUnusedLocalDefinitions(rootSym: gt.Symbol) {
        ...
        for (const currSym of this.currentSymbolContainer.members.values()) {
            ...
            if (
                currSym.flags & gt.SymbolFlags.Function &&
                currSym.valueDeclaration
            ) {
    ...
    private checkSourceElement(node: gt.Node) {
            ...
            if (this.currentSymbolContainer.declarations.length > 1) {
                ...
                if (node.kind === gt.SyntaxKind.FunctionDeclaration) {
                    for (const pd of this.currentSymbolContainer.declarations) {
                        if (pd === node) continue;
                        if (
                            pd.kind === gt.SyntaxKind.FunctionDeclaration &&
                            (!(<gt.FunctionDeclaration>pd).body ||
                    ...
                    previousDeclaration =
                        this.currentSymbolContainer.declarations[
                            this.currentSymbolContainer.declarations.length - 2
                        ];
                }

                if (previousDeclaration) {
                    const prevSourceFile = <gt.SourceFile>(
                        findAncestorByKind(
                            previousDeclaration,
                            gt.SyntaxKind.SourceFile,
                        )
                    );
                    const prevPos = getLineAndCharacterOfPosition(
                        prevSourceFile,
                        previousDeclaration.pos,
                    );
                    this.report(
                        (<gt.NamedDeclaration>node).name,
                        `Symbol redeclared, previous declaration in ${
                            prevSourceFile.fileName
                        }:${prevPos.line + 1},${prevPos.character + 1}`,
                    );
                }
            }
        }
        ...
        switch (node.kind) {
            ...
            case gt.SyntaxKind.ContinueStatement:
                this.checkBreakOrContinueStatement(
                    <gt.BreakOrContinueStatement>node,
                );
                break;
    ...
    private checkIncludeStatement(node: gt.IncludeStatement) {
        ...
        if (segments.length > 1 && segments[segments.length - 1] !== "galaxy") {
            this.report(
                node.path,
                'Dot in a script name is not allowed, unless path ends with ".galaxy"',
                gt.DiagnosticCategory.Warning,
            );
        } else {
        ...
        const currCourceFile = <gt.SourceFile>(
            findAncestorByKind(node, gt.SyntaxKind.SourceFile)
        );
    ...
    private checkFunction(node: gt.FunctionDeclaration) {
        ...
        for (const prevDecl of node.symbol.declarations) {
            ...
            const previousSignature = this.getSignatureOfFunction(
                <gt.FunctionDeclaration>prevDecl,
            );
            if (!currentSignature.match(previousSignature)) {
                this.report(
                    node,
                    `Function signature doesn't match it's previous declaration '${previousSignature.toString()}'`,
                );
                break;
            }
        }
    ...
    private checkLocalDeclaration(
        node:
            | gt.ParameterDeclaration
            | gt.VariableDeclaration
            | gt.PropertyDeclaration,
        symbol: gt.Symbol,
    ) {
        const sourceFile = <gt.SourceFile>(
            findAncestorByKind(node, gt.SyntaxKind.SourceFile)
        );
        ...
        if (
            ...
            if (globalSym) {
                if (
                    ...
                    const orgSourceFile = <gt.SourceFile>(
                        findAncestorByKind(
                            globalSym.declarations[0],
                            gt.SyntaxKind.SourceFile,
                        )
                    );
                    const orgPos = getLineAndCharacterOfPosition(
                        orgSourceFile,
                        globalSym.declarations[0].pos,
                    );
                    this.report(
                        node.name,
                        [
                            ...
                            `See: ${path.basename(
                                URI.parse(orgSourceFile.fileName).fsPath,
                            )}:${orgPos.line + 1},${orgPos.character + 1}`,
                        ].join(" "),
                    );
                }
            }
    ...
    private checkVariableDeclaration(node: gt.VariableDeclaration) {
        ...
        const isConstant = node.modifiers?.some(
            (value) => value.kind === gt.SyntaxKind.ConstKeyword,
        );
        if (isConstant && declType instanceof TypedefType) {
            this.report(
                node.type,
                "Constant variables cannot reference Typedefs",
            );
        }
        ...
        if (node.kind === gt.SyntaxKind.MappedType) {
            switch ((<gt.MappedTypeNode>node).returnType.kind) {
                ...
                case gt.SyntaxKind.ArrayrefKeyword: {
                    this.report(
                        node,
                        "Can not use arrayref/structref as a global, a field, or a return value (only as a local or a parameter).",
                    );
                    break;
                }
            }
    ...
    private checkIfStatement(node: gt.IfStatement) {
        ...
        if (node.elseStatement) {
            ...
            node.hasReturn =
                (<gt.Block>node.thenStatement).hasReturn &&
                (<gt.Block>node.elseStatement).hasReturn;
        }
    }
    ...
    private checkBreakOrContinueStatement(node: gt.BreakOrContinueStatement) {
        ...
        if (!loop) {
            this.report(
                node,
                `${tokenToString(
                    node.syntaxTokens[0].kind,
                )} statement used outside of loop boundaries`,
            );
        }
    }

    private checkReturnStatement(node: gt.ReturnStatement) {
        const fn = <gt.FunctionDeclaration>(
            findAncestorByKind(node, gt.SyntaxKind.FunctionDeclaration)
        );
    ...
    private checkMappedType(node: gt.MappedTypeNode) {
        if (!isReferenceKeywordKind(node.returnType.kind)) {
            this.report(
                node.returnType,
                "Invalid keyword for reference type provided - use funcref, arrayref or structref",
            );
        }
        ...
        if (node.typeArguments.length > 0) {
            ...
            switch (type.kind) {
                ...
                case gt.SyntaxKind.FuncrefKeyword:
                    invalid = !(
                        type.declaredType.flags & gt.TypeFlags.Function
                    );
                    break;
                case gt.SyntaxKind.ArrayrefKeyword:
            ...
            if (invalid) {
                this.report(
                    ...
                    "Type '" +
                        ...
                        "' is not a valid reference for '" +
                        tokenToString(node.returnType.kind) +
                        "'",
                );
            }
        }
    ...
    private checkExpressionWorker(
        ...
        switch (node.kind) {
            ...
            case gt.SyntaxKind.PropertyAccessExpression:
                return this.checkPropertyAccessExpression(
                    <gt.PropertyAccessExpression>node,
                );
            case gt.SyntaxKind.ElementAccessExpression:
                return this.checkIndexedAccess(
                    <gt.ElementAccessExpression>node,
                );
            ...
            case gt.SyntaxKind.ParenthesizedExpression:
                return this.checkParenthesizedExpression(
                    <gt.ParenthesizedExpression>node,
                    checkMode,
                );
            case gt.SyntaxKind.PrefixUnaryExpression:
                return this.checkPrefixUnaryExpression(
                    <gt.PrefixUnaryExpression>node,
                );
            case gt.SyntaxKind.PostfixUnaryExpression:
                return this.checkPostfixUnaryExpression(
                    <gt.PostfixUnaryExpression>node,
                );
            case gt.SyntaxKind.BinaryExpression:
                return this.checkBinaryExpression(
                    <gt.BinaryExpression>node,
                    checkMode,
                );
        }
        return unknownType;
        ...
        switch (node.kind) {
            case gt.SyntaxKind.StringLiteral:
                return new LiteralType(
                    gt.TypeFlags.StringLiteral |
                        gt.TypeFlags.String |
                        gt.TypeFlags.Nullable,
                    node,
                );
            case gt.SyntaxKind.NumericLiteral:
                if ((<gt.NumericLiteral>node).text.indexOf(".") !== -1) {
                    return new LiteralType(
                        gt.TypeFlags.NumericLiteral | gt.TypeFlags.Fixed,
                        node,
                    );
                } else {
                    return new LiteralType(
                        gt.TypeFlags.NumericLiteral | gt.TypeFlags.Integer,
                        node,
                    );
                }
            case gt.SyntaxKind.TrueKeyword:
    ...
    private checkBinaryExpression(
        ...
        if (isAssignmentOperator(node.operatorToken.kind)) {
            ...
            node.operatorToken.kind === gt.SyntaxKind.BarBarToken ||
            node.operatorToken.kind === gt.SyntaxKind.AmpersandAmpersandToken
        ) {
            this.checkTypeAssignableTo(leftType, boolType, node.left);
            this.checkTypeAssignableTo(rightType, boolType, node.right);
            return boolType;
        } else {
            const valid = leftType.isValidBinaryOperation(
            ...
            if (!valid) {
                this.report(
                    node,
                    [
                        `Binary expression '${tokenToString(
                            node.operatorToken.kind,
                        )}' not supported between '${leftType.getName()}' and '${rightType.getName()}'.`,
                    ].join(" "),
                );
            }
                ...
                case gt.SyntaxKind.SlashToken: {
                    if (leftType.flags & gt.TypeFlags.Numeric) {
                        ...
                        if (
                            ...
                            !(leftType.flags & gt.TypeFlags.Fixed)
                        ) {
                            return fixedType;
                        }
                        ...
                        if (
                            (leftType.flags & gt.TypeFlags.Byte &&
    ...
    private checkParenthesizedExpression(
        node: gt.ParenthesizedExpression,
        checkMode?: CheckMode,
    ) {
    ...
    private checkPrefixUnaryExpression(
        ...
        if (!type.isValidPrefixOperation(node.operator.kind)) {
            this.report(
                node,
                `Prefix '${tokenToString(
                    node.operator.kind,
                )}' operation not supported for '${type.getName()}' type`,
            );
        }
        return type;
    }

    private checkPostfixUnaryExpression(
        node: gt.PostfixUnaryExpression,
        checkMode?: CheckMode,
    ) {
    ...
    private checkIdentifier(
    ...
    ): [gt.Symbol, AbstractType] {
        const symbol =
            this.getSymbolOfEntityNameOrPropertyAccessExpression(node);
        if (!symbol) {
        ...
        if (symbol.flags & gt.SymbolFlags.Static) {
            ...
            if (symbol.parent && symbol.parent.declarations[0] !== sourceFile) {
                this.report(
                    node,
                    "Attempting to reference symbol with static modifier outside the scope of its definition.",
                );
            }
        }
        ...
        if (leftType !== unknownType) {
            ...
            if (fnType.flags & gt.TypeFlags.Function) {
                ...
                if (node.arguments.length !== func.parameters.length) {
                    this.report(
                        node,
                        `Expected ${func.parameters.length} arguments, got ${node.arguments.length}`,
                    );
                }
                ...
                this.report(
                    node,
                    `Type '${fnType.getName()}' is not calllable`,
                );
        ...
        if (func) {
            for (const [key, arg] of node.arguments.entries()) {
                ...
                if (func.parameters.length > key) {
                    const expectedType = this.getTypeFromTypeNode(
    ...
    private checkIndexedAccess(node: gt.ElementAccessExpression): AbstractType {
        ...
        if (
            !(indexType.flags & gt.TypeFlags.Integer) &&
            !(indexType.flags & gt.TypeFlags.NumericLiteral)
        ) {
            this.report(
                node.argumentExpression,
                "Array index require an integer value",
            );
        }
    ...
    private checkPropertyAccessExpression(
        node: gt.PropertyAccessExpression,
    ): AbstractType {
        ...
        if (!(type.flags & gt.TypeFlags.Struct)) {
            this.report(
                node.name,
                "Cannot access property on '" + type.getName() + "' type",
            );
        } else {
    ...
    private resolveName(
        location: gt.Node | undefined,
        name: string,
        limitScopeToDocument: boolean = false,
    ): gt.Symbol | undefined {
        if (location) {
            const currentContext = <gt.NamedDeclaration>(
                findAncestor(location, (element: gt.Node): boolean => {
                    return (
                        element.kind === gt.SyntaxKind.FunctionDeclaration ||
                        element.kind === gt.SyntaxKind.StructDeclaration
                    );
                })
            );
            ...
            const sourceFile = <gt.SourceFile>(
                findAncestorByKind(location, gt.SyntaxKind.SourceFile)
            );
        ...
        if (limitScopeToDocument && !location) {
            throw new Error(
                "Expected location when limitScopeToDocument is set to true",
            );
        }
    ...
    private resolveEntityName(
        entityName: gt.EntityNameExpression,
        meaning: gt.SymbolFlags,
        ...
        if (entityName.kind === gt.SyntaxKind.Identifier) {
            if (!this.isTypeIdentifier(entityName)) {
                ...
                symbol = this.resolveName(
                    findAncestorByKind(entityName, gt.SyntaxKind.SourceFile),
                    entityName.name,
                );
            }
        }
    ...
    private getSymbolOfEntityNameOrPropertyAccessExpression(
        entityName: gt.Identifier | gt.PropertyAccessExpression,
    ): gt.Symbol | undefined {
        ...
        if (entityName.kind === gt.SyntaxKind.Identifier) {
            ...
            this.checkPropertyAccessExpression(
                <gt.PropertyAccessExpression>entityName,
            ).symbol;
            return links.resolvedSymbol;
        }
    }
    ...
    public getSymbolAtLocation(node: gt.Node): gt.Symbol | undefined {
        switch (node.kind) {
            case gt.SyntaxKind.Identifier:
            case gt.SyntaxKind.PropertyAccessExpression:
                return this.getSymbolOfEntityNameOrPropertyAccessExpression(
                    <gt.Identifier | gt.PropertyAccessExpression>node,
                );
        }
    }
}
