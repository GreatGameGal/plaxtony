...
export const logger = createLogger({
    ...
    format: format.combine(
        ...
        format.printf((info) => {
            const out = [
                `${info.time} ${info.level.substr(0, 3).toUpperCase()} ${
                    info.message
                }`,
            ];
...
function formatMemoryUsage(mem: NodeJS.MemoryUsage) {
    ...
    return `${toMegabytes(mem.rss)}M ${toMegabytes(
        mem.heapTotal,
    )}M ${toMegabytes(mem.heapUsed)}M`;
}
...
export function logIt(lgOpts: LogItOptions = {}) {
    ...
    return function (
        ...
        function markDone(fnResult: any, pTimeSnapshot: [number, number]) {
            ...
            const out = [
                `-${msgPrefix.padEnd(35)} = ${diffMs.toFixed(0).padStart(4)}ms`,
            ];
