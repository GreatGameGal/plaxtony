...
describe("Service Definition", () => {
    ...
    before(async () => {
        await store.updateS2Workspace(
            new SC2Workspace(
                new SC2Archive(
                    "definition",
                    fixtureFilePath("service", "definition"),
                ),
            ),
        );
    });

    function getDefLinks(
        document: TextDocument,
        line: number,
        character: number,
    ) {
        return defProvider.getDefinitionAt(
            document.uri,
            getPositionOfLineAndCharacter(
                store.documents.get(document.uri),
                line,
                character,
            ),
        );
